import { Client, Producer, CompressionType } from '@apache-pulsar/pulsar-client'
...
export class PulsarEventBus implements NormalizedEventSink {
  ...
  private async getProducer(topic: string): Promise<Producer> {
    ...
    if (!producer) {
      producer = await this.client.createProducer({
        topic,
        compressionType: this.mapCompression(this.config.compressionType),
        sendTimeoutMs: 30000
      })
  ...
  private mapCompression(config?: PulsarEventBusConfig['compressionType']): CompressionType | undefined {
    switch (config) {
      case undefined:
        return undefined
      case 'NONE':
