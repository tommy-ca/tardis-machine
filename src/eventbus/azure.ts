...
const log = debug.extend('eventbus:azure')

const DEFAULT_BATCH_SIZE = 100 // Event Hubs max is 100 events per batch
const DEFAULT_BATCH_DELAY_MS = 25
const MAX_RETRY_ATTEMPTS = 3

export class AzureEventHubsEventBus implements NormalizedEventSink {
  ...
  constructor(private readonly config: AzureEventHubsEventBusConfig) {
  ...
  private flushImmediately() {
    ...
    this.sendingPromise = this.sendingPromise
      ...
      .catch((error) => {
        log('Failed to send Azure Event Hubs batch: %o', error)
  ...
  private async sendBatch(batch: BronzeEvent[]): Promise<void> {
    ...
    while (attempt < MAX_RETRY_ATTEMPTS) {
      ...
      try {
        ...
        log('Azure Event Hubs send attempt %d failed: %o', attempt, error)
  ...
  private async sendEvents(events: BronzeEvent[]): Promise<void> {
