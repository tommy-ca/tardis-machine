...
const cacheDir = './.cache-pubsub-e2e'
...
test('publishes replay-normalized events to Pub/Sub with Buf payloads', async () => {
  const options = {
    exchange: 'bitmex',
    symbols: ['ETHUSD'],
    from: '2019-06-01',
...
test('does not publish when Pub/Sub is not configured', async () => {
  ...
  await machineWithoutPubSub.start(8096)
  ...
  try {
    ...
    const params = encodeURIComponent(JSON.stringify(options))
    const response = await fetch(`http://localhost:8096/replay-normalized?options=${params}`)
